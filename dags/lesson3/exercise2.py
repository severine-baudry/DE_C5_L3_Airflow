...
def log_age_under_18():
     ...
     if len(records) > 0 and len(records[0]) > 0:
        ...
        logging.info("No rider under 18")
def log_younger_rider():
    redshift_hook = PostgresHook("redshift")
    records = redshift_hook.get_records("""
        SELECT birthyear FROM trips ORDER BY birthyear DESC LIMIT 1
    """)
    if len(records) > 0 and len(records[0]) > 0:
        logging.info(f"Yougest rider was born in {records[0][0]}")
...
def log_stations():
    redshift_hook = PostgresHook("redshift")
    records = redshift_hook.get_records("""
        SELECT city, count FROM city_station_counts ORDER BY count DESC LIMIT 10
    """)
    if len(records) > 0 and len(records[0]) > 0:
        logging.info(f"Nb stations by city :\n{records}")
...
log_stations_task = PythonOperator(
    task_id = "log_stations",
    dag = dag,
    python_callable = log_stations
)
log_younger_task = PythonOperator(
    task_id = "log_younger",
    dag = dag,
    python_callable = log_younger_rider
)
...
create_number_stations >> log_stations_task